...
public class FileUts {
	...
	public static void copyDirectory(
		...
		public static void copyDirectory(File source, File destination) {
			if (source.exists() && source.isDirectory()) {//如果文件存在并且是个目录
				if (!destination.exists()) {
					destination.mkdirs();
				}

				File[] fileArray = source.listFiles();//得到目录下所有的子列表（可能还有目录）

				for (int i = 0; i < fileArray.length; i++) {//递归
		...
		public static String read(String fileName, String rn) throws IOException {
			rn = CommFun.nvl(rn, StringPool.RN);
		...
		public static String replaceSeparator(String fileName) {
			return CommFun.replace(fileName, '\\', "/");
		}
	    ...
	    /**
		 * 根据唯一标识符pid,获取文件对应文档管理系统中的相对路径，相对文件系统的跟目录	
		 * PID编码=两位一级相对目录+两位二级相对目录+文件名摘要+13位时间序列+3位随机数+4位文件后缀名
		 * @param fPid 文件唯一标识符pid
		 * @return 文件路径
		 */
		...
		public static String genPid() {
			String pid = CommFun.getCurrDateFmt("yyyyMMddHHmmss"); // 生成PID
			pid += RandomUtil.genNum(3);
			return pid;
		}
		 ...
		 public static void main(String[] args) throws IOException {  
			   ...
			   String fileName = "D:/work/车帮帮/copy.txt";
//			   fileName = "D:/wamp/www/dep.txt";
			 
			   List<String> fileList = FileUts.toList(fileName);
			   
			   
			 
		       for (String relative_path : fileList ) {
		    	   String fdir  = relative_path.substring(0, relative_path.lastIndexOf("/"));
		    	   
		    	   fdir = fdir.replaceAll("D:/wamp/www/wcbbdepoly", "D:/wamp/www/actvs");
		    	   String destination = relative_path.replaceAll("D:/wamp/www/wcbbdepoly", "D:/wamp/www/actvs");
		    	   FileUts.MakeDirectory(fdir);
		    	   FileUts.copyFile(relative_path, destination);
		    	   System.out.println(fdir+""+relative_path);
		       }
